...
const Logs = ({
  ...
  const delayedQuery = useCallback(_.debounce((q) => {
    console.log(q)
    getLogs(q, functionId, startDate, endDate, showOnlyErrors, page, limit)
  }, 300), []);

  const getLogs = async (q, functionId, startDate, endDate, showOnlyErrors, page, limit) => {
    setLoading(true);
    try {
      const payload = {
  ...
  useEffect(() => {
    getLogs(query, functionId, startDate, endDate, showOnlyErrors, page, limit);
